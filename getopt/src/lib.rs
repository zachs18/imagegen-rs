...
impl Getopt {
    ...
    pub fn parse<'a, I: IntoIterator<Item = &'a str>>(&'a self, args: I) -> GetoptIter<'a, I::IntoIter> {
        GetoptIter { opts: &self.options, args: args.into_iter().peekable(), backlog: VecDeque::new(), found_dash_dash: false }
    }
    ...
    pub fn add_option(&mut self, opt: Opt) -> Result<(), (&'static str, Opt)> {
        ...
        } else if let Some(existing_opt) = self.options.iter().find(|e_opt| (e_opt.short.is_some() && e_opt.short == opt.short) || (e_opt.long.is_some() && e_opt.long == opt.long)) {
    ...
    pub fn from_iter(iter: impl IntoIterator<Item = Opt>) -> Result<Self, (&'static str, Opt)> {
        ...
        let mut this = Getopt { options: Vec::with_capacity(iter.size_hint().0) };
        for opt in iter {
            this.add_option(opt)?;
        }
        Ok(this)
...
impl<'a, I: Iterator<Item = &'a str>> Iterator for GetoptIter<'a, I> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if opt == "--" {
            ...
            let r_opt = match self.opts.iter().find(|r_opt| {
                Some(opt) == r_opt.long.as_deref()
            }) {
                Some(r_opt) => r_opt,
            ...
            match (r_opt.has_argument, arg) {
                ...
                (HasArgument::No, Some(_)) => Some(Err(GetoptError::UnrecognizedLongOpt { opt, arg })),
                // May require additional parsing
                (HasArgument::Yes, None) => {
                    match self.args.next() {
                        Some(arg) => Some(Ok(GetoptItem::Opt { opt: r_opt, arg: Some(arg) })),
                        None => Some(Err(GetoptError::UnrecognizedLongOpt { opt, arg })),
                    }
                },
                (HasArgument::Optional, None) => {
                    match self.args.peek() {
                        Some(arg) if !arg.starts_with('-') => Some(Ok(GetoptItem::Opt { opt: r_opt, arg: self.args.next() })),
                        Some(_) | None => Some(Ok(GetoptItem::Opt { opt: r_opt, arg: None })),
                    }
                },
            }
        } else if opt.starts_with("-") {
            ...
            loop {
                ...
                let r_opt = match self.opts.iter().find(|r_opt| {
                    Some(c_opt) == r_opt.short
                }) {
                    Some(r_opt) => r_opt,
                    None => {
                        // Only assume the unrecognized shortopt has an arg if its explicit with '='
                        if opt.as_str().starts_with('=') {
                            self.backlog.push_back(Err(GetoptError::UnrecognizedShortOpt { opt: c_opt, arg: Some(opt.as_str()) }));
                            break
                        } else {
                            self.backlog.push_back(Err(GetoptError::UnrecognizedShortOpt { opt: c_opt, arg: None }));
                            continue
                        }
                    },
                };
                ...
                match (r_opt.has_argument, opt.as_str()) {
                    (HasArgument::No, arg) if arg.starts_with('=') => {
                        self.backlog.push_back(Err(GetoptError::UnrecognizedShortOpt { opt: c_opt, arg: Some(&arg[1..]) }));
                        break
                    }
                    (HasArgument::No, _) => self.backlog.push_back(Ok(GetoptItem::Opt { opt: r_opt, arg: None })),
                    (HasArgument::Yes, arg) if arg.len() == 0 => {
                        self.backlog.push_back(match self.args.next() {
                            Some(arg) => Ok(GetoptItem::Opt { opt: r_opt, arg: Some(arg) }),
                            None => Err(GetoptError::UnrecognizedShortOpt { opt: c_opt, arg: None }),
                        });
                        break
                    }
                    (HasArgument::Yes, arg) if arg.starts_with('=') => {
                        self.backlog.push_back(Ok(GetoptItem::Opt { opt: r_opt, arg: Some(&arg[1..]) }));
                        break
                    },
                    (HasArgument::Yes, arg) => {
                        self.backlog.push_back(Ok(GetoptItem::Opt { opt: r_opt, arg: Some(arg) }));
                        break
                    },
                    (HasArgument::Optional, arg) if arg.len() == 0 => {
                        self.backlog.push_back(match self.args.peek() {
                            Some(arg) if !arg.starts_with('-') => Ok(GetoptItem::Opt { opt: r_opt, arg: self.args.next() }),
                            Some(_) | None => Ok(GetoptItem::Opt { opt: r_opt, arg: None }),
                        });
                        break
                    }
            ...
            self.next() // should use backlog, unless this was '-'
...
#[cfg(test)]
mod tests {
    use crate::{Getopt, Opt, HasArgument, GetoptItem, GetoptError};
    ...
    fn basic_short() {
        ...
        assert_eq!(getopt.parse(["-abc"]).collect::<Vec<_>>(), vec![
            Ok(GetoptItem::Opt { opt: &a, arg: None }),
            Ok(GetoptItem::Opt { opt: &b, arg: None }),
            Ok(GetoptItem::Opt { opt: &c, arg: None }),
        ]);

        assert_eq!(getopt.parse(["-abcarg=arg"]).collect::<Vec<_>>(), vec![
            Ok(GetoptItem::Opt { opt: &a, arg: None }),
            Ok(GetoptItem::Opt { opt: &b, arg: None }),
            Ok(GetoptItem::Opt { opt: &c, arg: Some("arg=arg") }),
        ]);

        assert_eq!(getopt.parse(["-abc=arg=arg"]).collect::<Vec<_>>(), vec![
            Ok(GetoptItem::Opt { opt: &a, arg: None }),
            Ok(GetoptItem::Opt { opt: &b, arg: None }),
            Ok(GetoptItem::Opt { opt: &c, arg: Some("arg=arg") }),
        ]);
    ...
    fn short_missing_arg() {
        ...
        assert_eq!(getopt.parse(["-ab", "arg=arg"]).collect::<Vec<_>>(), vec![
            Ok(GetoptItem::Opt { opt: &a, arg: None }),
            Ok(GetoptItem::Opt { opt: &b, arg: Some("arg=arg") }),
        ]);

        assert_eq!(getopt.parse(["-ab", "-carg=arg"]).collect::<Vec<_>>(), vec![
            Ok(GetoptItem::Opt { opt: &a, arg: None }),
            Ok(GetoptItem::Opt { opt: &b, arg: Some("-carg=arg") }),
        ]);

        assert_eq!(getopt.parse(["-ab"]).collect::<Vec<_>>(), vec![
            Ok(GetoptItem::Opt { opt: &a, arg: None }),
    ...
    fn basic_long() {
        ...
        assert_eq!(getopt.parse(["--a", "--b", "--c"]).collect::<Vec<_>>(), vec![
            Ok(GetoptItem::Opt { opt: &a, arg: None }),
            Ok(GetoptItem::Opt { opt: &b, arg: None }),
            Ok(GetoptItem::Opt { opt: &c, arg: None }),
        ]);

        assert_eq!(getopt.parse(["--a", "--b", "--c=arg=arg"]).collect::<Vec<_>>(), vec![
            Ok(GetoptItem::Opt { opt: &a, arg: None }),
            Ok(GetoptItem::Opt { opt: &b, arg: None }),
            Ok(GetoptItem::Opt { opt: &c, arg: Some("arg=arg") }),
        ]);
        ...
        assert_eq!(getopt.parse(["--a", "--b", "--carg=arg"]).collect::<Vec<_>>(), vec![
            Ok(GetoptItem::Opt { opt: &a, arg: None }),
            Ok(GetoptItem::Opt { opt: &b, arg: None }),
            Err(GetoptError::UnrecognizedLongOpt { opt: "carg", arg: Some("arg") }),
        ]);
    }
    ...
    fn long_missing_arg() {
        ...
        assert_eq!(getopt.parse(["--a", "--b"]).collect::<Vec<_>>(), vec![
            Ok(GetoptItem::Opt { opt: &a, arg: None }),
